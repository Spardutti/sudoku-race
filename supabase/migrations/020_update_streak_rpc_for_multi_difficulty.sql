-- Migration: Update Streak RPC for Multi-Difficulty
-- Story: 6.6 Multi-Difficulty Puzzle System
-- Generated: 2025-12-11
-- AC: 4.1-4.6

-- ============================================================================
-- STEP 1: Create new RPC that accepts difficulty parameter
-- ============================================================================

-- Drop old function (replaced with new signature)
DROP FUNCTION IF EXISTS update_user_streak(UUID, DATE);

-- New function with multi-difficulty support
CREATE OR REPLACE FUNCTION update_user_streak(
  ...
  perfect_day_streak INT,
  freeze_was_used BOOLEAN,
...
DECLARE
  ...
  v_new_perfect_day_streak INT;
  v_freeze_available BOOLEAN;
  v_last_freeze_reset_date DATE;
  v_days_since_freeze_reset INT;
  v_freeze_was_used BOOLEAN := false;
  v_streak_was_reset BOOLEAN := false;
  v_easy_completed BOOLEAN;
  v_medium_completed BOOLEAN;
BEGIN
  -- Get existing streak
  ...
  IF NOT FOUND THEN
    ...
    RETURN QUERY SELECT
      ...
      v_existing_streak.perfect_day_streak,
      v_freeze_was_used,
      v_streak_was_reset;
    RETURN;
  END IF;

  -- Same-day recompletion: check for perfect day
  IF v_existing_streak.last_completion_date = p_today THEN
    -- Check if both difficulties completed today
    SELECT
      EXISTS(SELECT 1 FROM completions c
             JOIN puzzles p ON c.puzzle_id = p.id
             WHERE c.user_id = p_user_id
             AND p.puzzle_date = p_today
             AND p.difficulty = 'easy'
             AND c.completed_at IS NOT NULL) AS easy_done,
      EXISTS(SELECT 1 FROM completions c
             JOIN puzzles p ON c.puzzle_id = p.id
             WHERE c.user_id = p_user_id
             AND p.puzzle_date = p_today
             AND p.difficulty = 'medium'
             AND c.completed_at IS NOT NULL) AS medium_done
    INTO v_easy_completed, v_medium_completed;

    -- Perfect Day: both completed
    v_new_perfect_day_streak := v_existing_streak.perfect_day_streak;
    IF v_easy_completed AND v_medium_completed THEN
      v_new_perfect_day_streak := v_new_perfect_day_streak + 1;

      UPDATE streaks
      SET perfect_day_streak = v_new_perfect_day_streak
      WHERE user_id = p_user_id
      RETURNING * INTO v_existing_streak;
    END IF;
  ...
  SET
    ...
    perfect_day_streak = v_new_perfect_day_streak
  WHERE user_id = p_user_id
  RETURNING * INTO v_existing_streak;
...
-- Add comments
COMMENT ON FUNCTION update_user_streak IS 'Updates user streak with multi-difficulty support (simple + perfect day streaks)';
